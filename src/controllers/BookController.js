        ...
        for (let i = 0; i < versionObj.length; i++) {
            ...
            for (let j in versionObj[i].chapters) {
                ...
                for (let l in versionObj[i].chapters[j]) {
                    ...
                    if(refferences['refference'] == null) report['refference'] = {};
                    ...
                    if(report['refference'] != null) await Book.create(report);
                    //const vs = await Book.create(report);

                    // Pushing to general report
                    genReport.push({ book_name:report['book_name'], ch_vs:report['ch_vs'], verse_text:report['verse_text'], refference:report['refference'] });
                }
            }
        }
